...
export class SurfaceDetection extends BaseScriptComponent {
  ...
  private update() {
    ...
    const rayStart = camPos.add(
      rayDirection.uniformScale(-this.MIN_HIT_DISTANCE)
    );
    const rayEnd = camPos.add(
      rayDirection.uniformScale(-this.MAX_HIT_DISTANCE)
    );
  ...
  private updateCalibration(foundPosition: vec3, foundNormal: vec3) {
    ...
    if (foundNormal.distance(vec3.up()) < 0.1) {
      ...
      const worldCameraForward = this.camTrans.right
        .cross(vec3.up())
        .normalize();
      ...
      const distance = this.history[0].distance(
    ...
    //interpolate
    this.visualTrans.setWorldPosition(
      vec3.lerp(currPosition, this.desiredPosition, getDeltaTime() * this.SPEED)
    );
    this.visualTrans.setWorldRotation(
      quat.slerp(
        currRotation,
  ...
  private onCalibrationComplete() {
    ...
    this.onGroundFoundCallback?.(
      this.calibrationPosition,
