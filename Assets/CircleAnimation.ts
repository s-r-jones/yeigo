...
export class CircleAnimation extends BaseScriptComponent {
  ...
  private update() {
    ...
    if (currAmount < 0.01) {
      this.calRenderer.mainPass.CurrSize = this.PingPong(
        this.startSize * 2,
        this.startSize * 3,
        getTime() * 0.75
      );
      ...
      this.calRenderer.mainPass.CurrSize = this.PingPong(
        0.095,
        0,
        (getTime() - this.startCompleteAnimTime) * 1
      );
    ...
    this.calRenderer.mainPass.Amount = MathUtils.lerp(
      currAmount,
      this.desiredLoadAmount,
      getDeltaTime() * 6
    );
  }
